...
def test_extract_phenopackets(temp_dir, mock_phenopacket_zip, monkeypatch):
    ...
    # Create a mock for the extract_directly method with the correct signature
    ...
    # Setup output directory in temporary directory
    output_dir = temp_dir / "data"
    output_dir.mkdir(exist_ok=True)
    
    registry = PhenopacketRegistryService(data_dir=temp_dir)
    
    # Set the configuration to use our test directory
    registry.config.output_dir = str(output_dir)
    
    # Apply the monkey patches
    monkeypatch.setattr(registry, "download_latest_release", mock_download)
    monkeypatch.setattr(PhenopacketRegistryService, "_extract_directly", mock_extract_directly)
